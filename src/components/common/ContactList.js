...
import ContactDeletePrompt from "./ContactDeletePrompt";
  ...
  render() {
    ...
    return (
      <div>
        <Table striped bordered hover variant="dark">
          ...
          {users.map((user, index) => {
            if (index >= start_offset && start_count < per_page) {
              start_count++;
              return (
                <ContactListElement key={index} user={user} showDelete={this.showDelete}/>
              );
            }
          })}
          </tbody>
        ...
        <ContactDeletePrompt show={this.state.delete_show} user={this.state.delete_user}
